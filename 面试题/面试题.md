...
例如mysql 只支持环形拓扑 其中每个节点接受来自前序节点的吸入，并将这些写入转发给后序节点。

星型拓扑：一个指定的根节点将写入转发给其他所有其他节点。

为防止无限循环，每个几点需要赋予一个唯一的标识符。

在复制日志的每个写请求都标记了已通过的节点标识符。

如果包含自身标识符的数据更改，表情该请求已经被处理过了，就忽略请求，避免重复转发。

环形和星型拓扑的问题：

如果一个节点发生了故障，在修复之前，会影响其他节点之间复制日志的转发。

解决方案:

可以采用重新配置拓扑结构的方法，暂时排除掉故障节点。



对于全部到全部的额拓扑，消息可以沿着不同路径传播，避免了单点故障



全链路拓扑也存在以问题i:主要是存在某些网络练练路比其他链路更快导致复制日志之间的覆盖。

为了保证日志消息正确有序，可以采用一种版本向量的技术。
...
[![img](https://camo.githubusercontent.com/385f07ad59299d08a559cd0e72248ab92a680ef3defa1f55d94ad6cb4126c7c1/68747470733a2f2f706963332e7a68696d672e636f6d2f76322d39346330633133396266613037313632323261333431333934623831303663655f622e6a7067)](https://camo.githubusercontent.com/385f07ad59299d08a559cd0e72248ab92a680ef3defa1f55d94ad6cb4126c7c1/68747470733a2f2f706963332e7a68696d672e636f6d2f76322d39346330633133396266613037313632323261333431333934623831303663655f622e6a7067)

### 1.Session复制

[![img](https://camo.githubusercontent.com/39f0e165014a6021222db28519b815c4a20bd093add3c9d23726285e95a74bf5/68747470733a2f2f706963342e7a68696d672e636f6d2f76322d64356635376562346536623135353639636532666564643038653132333264375f622e6a7067)](https://camo.githubusercontent.com/39f0e165014a6021222db28519b815c4a20bd093add3c9d23726285e95a74bf5/68747470733a2f2f706963342e7a68696d672e636f6d2f76322d64356635376562346536623135353639636532666564643038653132333264375f622e6a7067)
...
2.但是假设我们有 **N** 台机器，那么每次复制都要复制给 **N-1** 台机器，如果机器很多，可能会形成网络风暴，复制性能也会呈指数级下降。

3.Tomcat需要保存所有的Session数据，这个方案的 **Session** 存储在**内存**中，容易受到机器的总内存的限制。我们没办法通过加机器的方式**水平扩展**，我们能做的方式就是加大机器内存。但是机器内存越大，价格真的很贵！！！

实现方式：

① 设置tomcat ，server.xml 开启tomcat集群功能

[![img](https://github.com/xsjzn/backendKnowldge/raw/dev/%E9%9D%A2%E8%AF%95%E9%A2%98)](https://github.com/xsjzn/backendKnowldge/blob/dev/面试题)
...
这样，每个用户浏览器存储自己的 **Cookie** 信息，服务端就不需要存储，这就解决了 Session [复制方案](https://www.zhihu.com/search?q=复制方案&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"article"%2C"sourceId"%3A"161613562"})的缺陷了。

接下来用户每次请求都把这个 **Cookie** 给我发过来，我判断 **Cookie** 里面用户信息不就好了。

[![img](https://camo.githubusercontent.com/424020505336bb37b06b2ca13596328bcbcd16125c65885390fe9367ad93a629/68747470733a2f2f706963342e7a68696d672e636f6d2f76322d34633231636534303065346362353362356233373336623536663538663766335f622e6a7067)](https://camo.githubusercontent.com/424020505336bb37b06b2ca13596328bcbcd16125c65885390fe9367ad93a629/68747470733a2f2f706963342e7a68696d672e636f6d2f76322d34633231636534303065346362353362356233373336623536663538663766335f622e6a7067)

缺点：

1.每次请求都要携带cookie，占用外网带宽，如果cookie过大，会增大网络开销。

2.存储的数据大小容易受到Cookie限制

3.最关键的是要对用户信息进行加密，否则存在安全问题，造成信息泄漏
...
[![img](https://camo.githubusercontent.com/36b7559fc8ee247f13ff6aafc401b6f1fafc3f6e5e8cd310db1cab74ba47f0c8/68747470733a2f2f706963332e7a68696d672e636f6d2f76322d65383430323264343263346664643032383661356638353863323933623534325f622e6a7067)](https://camo.githubusercontent.com/36b7559fc8ee247f13ff6aafc401b6f1fafc3f6e5e8cd310db1cab74ba47f0c8/68747470733a2f2f706963332e7a68696d672e636f6d2f76322d65383430323264343263346664643032383661356638353863323933623534325f622e6a7067)

上面这种方式我们使用 Nginx 四层负载均衡方式，其实 Nginx 还可以做到七层负载均衡方式，也就是使用 Http 协议中的一些**业务属性**来做 Hash，常见的有 *userId*,*loginId*等等。

架构图如下：

[![img](https://camo.githubusercontent.com/c2e6eafb0c719b627ebec11c1a10c5c58dbe4c01fafa572931174f0d4aec7134/68747470733a2f2f706963312e7a68696d672e636f6d2f76322d65313432313739613931633063333934313430616231646239333537636130385f622e6a7067)](https://camo.githubusercontent.com/c2e6eafb0c719b627ebec11c1a10c5c58dbe4c01fafa572931174f0d4aec7134/68747470733a2f2f706963312e7a68696d672e636f6d2f76322d65313432313739613931633063333934313430616231646239333537636130385f622e6a7067)
...
1.增加新机器，修改完 Nginx 配置，重新启动之后，Nginx 将会重新计算 Hash 分发请求。因此需要重新登陆

2.Tomcat重启，Session由于是防止在内存中，所以这台机器上的Session将会丢失，这部分用户需要重新登陆。
...
[![img](https://camo.githubusercontent.com/7b3bb91df8b7cf7ad47001bc3784ce9a92a4c69bc37bd295e5c46b21d6699291/68747470733a2f2f706963332e7a68696d672e636f6d2f76322d35636439346464376564356536646336313266346662396634306138316237325f622e6a7067)](https://camo.githubusercontent.com/7b3bb91df8b7cf7ad47001bc3784ce9a92a4c69bc37bd295e5c46b21d6699291/68747470733a2f2f706963332e7a68696d672e636f6d2f76322d35636439346464376564356536646336313266346662396634306138316237325f622e6a7067)

优点：

1.安全 服务器出现问题，session不会丢失
...
小结 以上讲述的就是集群或分布式环境下，session的5种处理策略。其中就应用广泛性而言，第三种方式，也就是基于第三方缓存框架共享session，应用的最为广泛，无论是效率还是扩展性都很好。而Terracotta作为一个JVM级的开源群集框架，不仅提供HTTP Session复制，它还能做分布式缓存，POJO群集，跨越群集的JVM来实现分布式应用程序协调等，也值得学习一下。

## 四层负载均衡(Layer 4 Proxy)和七层负载均衡(Layer 7 Proxy)的区别

**四层负载均衡( L4 load balancing ):**

主要工作处于OSI模型中间位置的**传输层(transport layer)**，它主要处理消息的传递，而不管消息的内容。

在互联网上，TCP就是HTTP传输方式的四层协议( Layer 4 Protocol )。**四层负载均衡只针对由上游服务发送和接受的网络报包，而不检查包内的具体内容是什么。**

四层负载均衡可以通过检查TCP流中的前几个包，从而决定是否限制路由。

**七层负载均衡( L7 load balancing ):**

主要工作于处于OSI模型顶层位置的[应用层](https://www.zhihu.com/search?q=应用层&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"article"%2C"sourceId"%3A"53438208"})( application layer )，它主要处理每条消息中的真正内容。在互联网上，HTTP是网络通讯中占据主导地位的七层协议( Layer 7 Protocol )。

七层负载均衡在网络传输时比四层均衡更加复杂巧妙，特别适合像http这种机遇TCP传输的方式。

**一个七层负载均衡器终止网络传输并读取消息中的内容。它可以基于消息中内容( 比如URL或者cookie中的信息 )来做出负载均衡的决定。**之后，七层负载均衡器建立一个新的TCP链接来选择上游服务。( 或者再利用一个已经存在的TCP连接，通过 HTTP keepalives 的方式，见:[https://www.nginx.com/blog/http-keepalives-and-web-performance/](https://link.zhihu.com/?target=https%3A//www.nginx.com/blog/http-keepalives-and-web-performance/) )并向这个服务发出请求
...
使用七层负载均衡的设备经常被用于**反向代理**( reverse-proxy server 见: [https://www.nginx.com/resources/glossary/reverse-proxy-server/](https://link.zhihu.com/?target=https%3A//www.nginx.com/resources/glossary/reverse-proxy-server/) )。

## 一个七层负载均衡的例子

让我们来看一个简单的例子吧：用户访问一个繁忙的网站。在这个用户 session 的航向上，他或她可能会请求静态内容——比如图片或者视频，动态内容——比如[新闻递送](https://www.zhihu.com/search?q=新闻递送&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={"sourceType"%3A"article"%2C"sourceId"%3A"53438208"})，甚至是事务型信息——比如外卖点单的状态。七层负载均衡允许均衡器依据请求自身的信息进行路由，比如被请求内容的类型。所以现在一个针对图片和视频的请求可以被路由到存储并高度优化的多媒体内容服务器上。对于事物型信息比如商品折后价，可以被路由到响应管理价格的应用服务器上。用了七层负载均衡，网络和应用的架构师可以建立一个高速调整且高度优化的、针对需求可靠且可有效延展的服务基础架构或应用递送网络。

作者：Leon Tian 链接：https://zhuanlan.zhihu.com/p/53438208 来源：知乎 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。

## RPC注册中心选型：

注册中心提供服务的注册发现，用来调用链路的Provider和Consumer这两个端点。一个注册中心得管理好服务的扩缩容 故障转移 流量分配这样的核心功能。自身同样需要高可用，妥善解决跨地域部署情况下注册中心节点之间的数据一致性，因此我更倾向于认为注册中心是一个偏向体系化的产品，它的价值依赖于与之相匹配的服务规模和治理体系。
...
TreeMap Hashmap 和 linkedhashmap的使用场景

1.基于排序的统计功能

此时对于增删改查以及统计的时间复杂度都控制在O(logn)的级别上，相对于HashMap和LikedHashMap的统计操作的(最大的key，最小的key，大于某一个key的所有Entry等等)时间复杂度O(n)具有较高时间效率。

2.需要存储功能

hashmap和linkedhashmap的查询时间复杂度O(1)

3.需要快速增删改查而且需要保证便利和插入顺序一致的存储功能
...
Mq你学习的的时候可以回答这几个问题 1. 如何保证消息不丢 2. 如何保证消息不重复消费 3. 如何保证消息有序 4. 事物消息原理，和普通消息的异同 5. 延迟消息的原理，和普通消息的异同 6. 集群消费消息时，负载均衡算法如何实现
